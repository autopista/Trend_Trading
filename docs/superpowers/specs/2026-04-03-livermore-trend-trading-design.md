...
│
├── tests/                     # 단위 + 통합 테스트
│   ├── test_market_key.py     # 6단계 전환 로직 테스트
│   ├── test_pivot_points.py   # 피봇 감지 + false breakout 필터 테스트
│   ├── test_signal_generator.py # 시그널 생성 + 우선순위 테스트
...
CREATE INDEX idx_prices_market ON prices (market);
CREATE INDEX idx_market_indices_name_date ON market_indices (index_name, date);
CREATE INDEX idx_livermore_states_symbol_date ON livermore_states (symbol, date);
CREATE INDEX idx_signals_symbol_date ON signals (symbol, date);
...
CREATE INDEX idx_portfolio_market_date ON portfolio (market, date);
```
...
| reference_pivot_price | REAL | 현재 추세의 기준 피봇 가격 |
| trend_direction | TEXT | 추세 방향 (up, down, neutral) |
| trend_strength | REAL | 추세 강도 (0~100, 피봇 이격률 + 거래량 + 지속기간 종합) |
| trend_duration_days | INTEGER | 현재 추세 지속 일수 |
| UNIQUE(symbol, date) | | |
...
**출력**: 날짜별 column_state, reference_pivot_price, trend_direction, trend_strength, trend_duration_days
...
**False Breakout 필터링**:
- 돌파 확정 조건: 돌파 후 2거래일 연속 해당 가격대 이상 종가 유지
- 이탈 확정 조건: 이탈 후 2거래일 연속 해당 가격대 이하 종가 유지
- 확정 전까지는 'watch' 시그널로 분류, 확정 시 'buy'/'sell'로 전환

**시간 요소 분석**:
- 횡보 기간 측정: 피봇 근처에서의 횡보 기간이 길수록 돌파 시 신뢰도 가산
- 추세 지속 기간 추적: `trend_duration_days`로 현재 추세가 얼마나 유지되었는지 기록
...
class BaseCollector(ABC):
    ...
    def validate_ohlcv(self, df: pd.DataFrame) -> pd.DataFrame:
        """데이터 유효성 검증: low <= open/close <= high, volume >= 0, 결측치 처리"""
```

**에러 처리 및 재시도:**
- 모든 API 호출에 지수 백오프(Exponential Backoff) 재시도 적용 (최대 3회)
- 네트워크 오류, Rate Limit 초과 시 자동 재시도
- 수집 실패한 종목은 로그에 기록하고 다음 종목으로 진행
- Python `logging` 모듈로 상세 로깅 (수집 건수, 소요 시간, 실패 종목)

**데이터 검증:**
- `low <= open`, `low <= close`, `open <= high`, `close <= high` 확인
- `volume >= 0` 확인
- 결측치(NaN) 발견 시 전일 데이터로 보간 또는 해당 날짜 제외 후 경고 로깅
...
**시그널 타입:**
- **buy**: 매수 시그널 (피봇 돌파 확정 + 거래량 확인 + 시장 추세 일치 + RSI 과매수 아님)
- **sell**: 매도 시그널 (추세 전환 확정 + 손절선 이탈)
- **watch**: 관찰 — 아래 조건 중 하나 이상 해당:
  - 피봇 돌파/이탈 발생했으나 2일 확정 대기 중
  - 거래량 미확인 (평균 대비 1.5배 미달)
  - 시장 추세가 중립이어서 강한 시그널만 허용 상태
  - RSI가 과매수(>70)/과매도(<30) 구간
- **watch → buy/sell 전환 조건**: 2일 확정 완료 + 거래량 확인 + 시장 추세 일치
...
- 시장 추세 일치도: 0~20점
- 리버모어 상태 강도: 0~15점
- RSI 위치 적정성: 0~10점 (과매수/과매도 구간이면 감점)
- 시간 요소: 0~10점 (횡보 기간 길수록, 추세 지속 기간 적정할수록 가산)

**다중 시그널 우선순위:**
동시에 여러 종목 시그널 발생 시 다음 순서로 우선순위 결정:
1. 신뢰도 높은 시그널 우선
2. 동일 신뢰도면 시장 추세 강도가 강한 쪽 우선
3. 동일하면 거래량 급증 비율이 높은 쪽 우선
...
```
Phase 1: 데이터 수집
  └── collectors/ → 시장 지수 + 종목 OHLCV → DB 저장 → 데이터 검증
...
**에러 처리 정책:**
- Phase 1 실패 시: 해당 시장 Phase 2~4 중단, 텔레그램으로 관리자 알림
- Phase 2 실패 시: 해당 종목 건너뛰고 다음 종목 진행, 실패 종목 로깅
- Phase 3 실패 시: 시그널은 DB에 저장하되 알림 실패는 재시도 큐에 추가
- Phase 4 실패 시: 경고 로깅 후 다음 실행 시 자동 복구
- 모든 Phase 실행 결과(성공/실패, 처리 건수, 소요 시간)를 로깅
- 대시보드에 "마지막 업데이트 시각" + "시스템 상태(정상/오류)" 표시
...
### 백테스팅 모듈 (우선 구현 권장)
- `backtesting/` 디렉토리 추가
- `livermore_engine`과 `signals` 모듈을 재사용하여 과거 데이터 시뮬레이션
- 백테스팅 결과: 수익률, MDD, 승률, 손익비 산출
- `settings.yaml` 파라미터 최적화에 활용 (과최적화 방지를 위해 walk-forward 검증)
- 백테스팅 결과를 `web/performance.html`에 통합 시각화
